...
class Workbook:
    ...
    def open_session(self):
        if not self.__workbook:
            try:
                ...
                print(f"Workbook {self.filePath} loaded successfully.")
            except FileNotFoundError:
                print(f"File {self.filePath} not found. Loading default file.")
    ...
    def saveWorkbook(self, fileName):
        if self.__workbook is None:
            return False
        self.__workbook.properties.creator = "Solayof Pragmatics ICT Hub"
        self.__workbook.properties.description = (
            "This Excel file was automatically generated by a Streamlit-based "
            "data recording application built in Python. The system integrates "
            "SQLAlchemy with SQLite for data management and uses Openpyxl and "
            "Pandas to process and write records into the workbook. "
    ...
    def get_subject_dict(self, sheetName):
        ...
        while db[cell].value:
            ...
            if db_cell.value.startswith('='):
                sub_cells[f'{db[db_cell.value[1:]].value}'] = db_cell.coordinate
            else:
